        ...
        <v-card>
          ...
          <v-card-text>
            <v-form>
              ...
              <v-text-field type="password" :disabled="disabled" v-model="user.repeatPassword"
                            label="Repeat Password" required/>

              <v-btn class="mr-4" color="success" :disabled="disabled" @click="submit">Sign up</v-btn>
            </v-form>
          </v-card-text>
        </v-card>
      </v-col>
...
export default defineComponent({
  ...
  methods: {
    ...
    submit() {
      ...
      this.httpClient.post('/register', this.user).then(response => {
